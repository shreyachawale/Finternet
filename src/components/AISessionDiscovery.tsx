...
export default function AISessionDiscovery({ onNavigate }: AISessionDiscoveryProps) {
  ...
  const [chatHistory, setChatHistory] = useState<ChatMsg[]>([
    {
      type: 'ai',
      text:
        'Tell me what you want to learn. I’ll recommend recorded sessions where you pay only for what you actually watch.'
    }
  ]);
  ...
  const handleSend = () => {
    ...
    const results = semanticSearch(message);
    ...
    setTimeout(() => {
      setChatHistory(prev => [
        ...prev,
        {
          ...
          text:
            results.length > 0
              ? `I found ${results.length} recorded sessions that match your goal.
Most learners get the best value in the first ${results[0].recommended_min}–${results[0].recommended_max} minutes.`
              : 'I couldn’t find a close match yet. Try describing your goal differently.'
        }
      ]);
      setSessions(results);
    }, 600);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-100">
      ...
      <div className="max-w-6xl mx-auto px-8 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* AI GUIDE */}
          <div className="bg-white/60 backdrop-blur-xl rounded-3xl border shadow-xl p-6">
            ...
            <div className="flex space-x-3">
              <input
                ...
                placeholder="e.g. I want to learn jazz piano basics"
          ...
          <div className="space-y-4">
            {sessions.map(session => (
              <div
                key={session.id}
                className="bg-white/60 backdrop-blur-xl rounded-2xl border shadow-lg p-6"
              >
                <div className="flex justify-between mb-4">
                  <div className="flex space-x-4">
                    <div className="w-14 h-14 bg-blue-500 rounded-xl text-white flex items-center justify-center font-bold">
                      {session.avatar}
                    </div>
                    <div>
                      <div className="flex items-center space-x-2">
                        <h3 className="font-bold">{session.teacher}</h3>
                        {session.verified && (
                          <Shield className="w-4 h-4 text-blue-500" />
                        )}
                      </div>
                ...
                {/* DETAILED INFO */}
                <div className="bg-blue-50 rounded-xl p-4 text-sm text-blue-700 mb-4 space-y-2">
                  {session.why_this_fits && (
                    <div>
                      💡 <strong>Why this fits:</strong> {session.why_this_fits}
                    </div>
                  )}

                  <div className="grid grid-cols-2 gap-2 text-xs text-blue-800">
                    <div>📘 Recorded • Resume anytime</div>
                    <div>
                      ⏱ Best value: {session.recommended_min}–
                      {session.recommended_max} min
                    </div>
                    <div>
                      💰 Max charge: ~$
                      {session.max_charge?.toFixed(2)}
                    </div>
